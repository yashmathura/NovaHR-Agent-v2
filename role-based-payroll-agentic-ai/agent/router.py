...
INTENTS = {
    ...
    "assign_task": [
        r"\b(assign|create)\b.*\b(task|work)\b"
    ],
    ...
    "apply_leave": [
        r"\b(apply|request|take)\b.*\b(leave|vacation|time off)\b",
        r"need.*leave"
    ],
    ...
    "get_team_attendance": [
        ...
        r"who.*\b(absent|present)\b.*today"
    ],

    "get_leave_balance": [
        r"\b(leaves?|vacation|time off)\b.*\b(balance|left|remaining|available)\b",
        r"how many (leaves?|days)"
    ],
    ...
    "get_attendance": [
        r"\b(attendance|attendance history|attendance report)\b",
        r"how many days.*present",
        r"am i absent"
    ],
    ...
    "mark_notifications_read": [
        r"\b(mark|clear)\b.*\b(notification|notifications)\b.*\bread"
    ],

    "profile": [
        r"\b(my profile|my details|my information|who am i)\b"
    ],
...
def resolve_intent(text):
    ...
    # ---------------------------------------------------------
    # LAYER 1: Local NLP semantic classifier
    # ---------------------------------------------------------

    nlp_result = classify(t)

    nlp_intent = nlp_result.get("intent")
    nlp_confidence = nlp_result.get("confidence", 0)

    # High-confidence NLP prediction
    if nlp_intent in INTENTS and nlp_confidence >= 0.55:
        return nlp_intent

    # ---------------------------------------------------------
    # LAYER 2: NovaLM learned local language model
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # LAYER 3: Deterministic safety router
    # ---------------------------------------------------------
...
def extract_priority(text):
    ...
    for x in ("LOW", "MEDIUM", "HIGH", "URGENT"):
        ...
        if x in t:
            return "HIGH" if x == "URGENT" else x
